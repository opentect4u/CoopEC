...
WdtlsRouter.post("/saveuser", async (req, res) => {
  try {
    ...
    var fields =
      data.id > 0
        ? `user_name = '${data.user_name.split("'").join("\\'")}',user_email = '${data.user_email}',designation = '${data.designation}',user_type = '${data.user_type}',
                  ...
                  modified_ip = '${ip}' `
        : `(user_id,user_name,user_email,designation,user_type,password,user_status,cntr_auth_type,range_id,created_at,created_by,created_ip)`;
    var whr = `id = '${data.id}'`;
    var flag = data.id > 0 ? 1 : 0;
    var save_data = await db_Insert(table_name, fields, values, whr, flag);
