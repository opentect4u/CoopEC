...
//       { header: "Election Status", key: "election_status" },
//     ];
...
//           audit_upto: item.audit_upto, // Convert date
...
//   }
// });
DashboardRouter.get("/boardmember_download", async (req, res) => {
  try {
    var soc_id = `b.soc_id=${req.query.soc_id}`;
    var soc_name = "";
    ...
    const soc_res_dt = await db_Select(select_soc, table_soc, `a.id=${req.query.soc_id}`, null);

    // Create a new workbook and worksheet
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet("Report");
    const headerRow = worksheet.addRow([
      "Society Name", "Registration No", "Registration Date", "Society Type", 
      "Tier Name", "Controlling Authority Type", "Returning Officer", "State", 
      "District", "Zone", "Range", "Urban/Rural", "ULB Category", "ULB Name", 
      "Ward", "Block", "Gram Panchayat", "Village", "PIN No", "Address", 
      "Management Status", "Officer Type", "Number of Members", "Audit Up To", 
      "Last Election Date", "Tenure Ends On", "Key Person", "Designation", 
      "Contact Number", "Email", "Case ID", "Case Number", "Functional Status", 
      "Election Status"
    ]);
    headerRow.eachCell((cell) => {
      cell.font = { bold: true, size: 12 };
    });
    // Define columns for Society Details
    worksheet.columns = [
      { header: "Society Name", key: "cop_soc_name" },
      { header: "Registration No", key: "reg_no" },
      { header: "Registration Date", key: "reg_date" },
    ...
    // Add Society Details
    var results = soc_res_dt.suc > 0 ? soc_res_dt.msg : [];
    results.forEach((item) => {
      worksheet.addRow(item);
      soc_name = item.cop_soc_name;
    });

    // Fetch Board Member Details
    const select =
      "a.cop_soc_name,b.board_memb_name,b.board_memb_desig,b.board_memb_father,b.board_memb_email,b.bm_contact_no";
    const table_name = `md_society a JOIN td_board_member b ON a.id = b.soc_id`;
    const res_dt = await db_Select(select, table_name, `b.soc_id=${req.query.soc_id}`, null);

    console.log("Board Members Data:", res_dt);

    // Add a gap and Board Member Headers manually
    worksheet.addRow([]);
    worksheet.addRow(["Board Member Name", "Board Member Designation", "Board Member Father", "Board Member Email", "Board Member Contact"]).font = { bold: true, size: 14 };

    var result = res_dt.suc > 0 ? res_dt.msg : [];

    // Add Board Member Rows
    result.forEach((item) => {
      worksheet.addRow([
        item.board_memb_name,
        item.board_memb_desig,
        item.board_memb_father,
        item.board_memb_email,
        item.bm_contact_no,
      ]);
    });
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=Board_member_list_of_${soc_name}.xlsx`
    );
...
DashboardRouter.get("/society_download", async (req, res) => {
  try {
    ...
    const select =
      "a.id as socid,a.cop_soc_name, a.reg_no, a.reg_date, b.soc_type_name, f.soc_tier_name, h.controlling_authority_type_name AS reg_cont_auth, g.controlling_authority_name AS returning_officer, st.state_name, c.dist_name, d.zone_name, e.range_name, a.urban_rural_flag, ulcat.ulb_catg_name, ulb.ulb_name, wa.ward_name, mb.block_name, gp.gp_name, vill.vill_name, a.pin_no, a.address, mms.manage_status_name, mot.officer_type_name, a.num_of_memb, a.audit_upto, a.last_elec_date, a.tenure_ends_on, a.contact_name AS key_person, a.contact_designation AS key_person_desig, a.contact_number, a.email, a.case_id, a.case_num, a.functional_status,a.election_status";
    const table_name = `md_society a 
          LEFT JOIN md_society_type b ON a.soc_type = b.soc_type_id 
          LEFT JOIN md_district c ON a.dist_code = c.dist_code 
          LEFT JOIN md_controlling_authority_type h ON a.cntr_auth_type = h.controlling_authority_type_id 
    ...
    worksheet.columns = [
      ...
      { header: "Functional Status", key: "functional_status" },
      { header: "Election Status", key: "election_status" },
      { header: "Board Member detail", key: "board_menber_detail" },
    ];
    var result = res_dt.suc > 0 ? res_dt.msg : "";
    const boardMembersMap = {};

  for (const dk of result) { // Use for...of to handle async properly
    const board_members_res = await db_Select(
      "soc_id,board_memb_name, board_memb_desig,board_memb_father,board_memb_email,bm_contact_no",
      "td_board_member",
      `soc_id='${dk.socid}'`,
      null
    );

    if (board_members_res.msg.length > 0) {
      board_members_res.msg.forEach((item) => {
        const socid = item.soc_id; // Ensure socid is present
        const boardMemberDetail = `${item.board_memb_name}-${item.board_memb_desig}-${item.board_memb_father}-${item.board_memb_email}-${item.bm_contact_no}`;
        // Initialize the socid key if it doesn't exist
        if (!boardMembersMap[socid]) {
          boardMembersMap[socid] = boardMemberDetail;
        } else {
          boardMembersMap[socid] += `, ${boardMemberDetail}`;
        }
      });
    ...
    result.forEach(async (item) => {
      ...
      worksheet.addRow({
          ...
          functional_status: item.functional_status,
          election_status: item.election_status,
          board_menber_detail: boardMembersMap[item.socid] || ""
      });
  });
