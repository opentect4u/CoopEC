...
WapiRouter.post("/getsoctypegrouplist", async (req, res) => {
  ...
  var range_List = await db_Select(
    "range_id,range_name",
    "md_range",
  ...
  if (range_List.msg.length > 1) {
    ...
    var range1_tot = await db_Select(
      `count(*) as tot`,
      table_name1_for_tot,
      where,
      null,
    );
    var range2_tot = await db_Select(
      `count(*) as tot`,
      table_name2_for_tot,
      where,
      null,
    ...
    var range1 = {
      range_name: range1_name,
      range_code: range1,
      range_tot: range1_tot.msg[0].tot,
      range_data: res_dt1.msg,
    };
    var range2 = {
      range_name: range2_name,
      range_code: range2,
      range_tot: range2_tot.msg[0].tot,
      range_data: res_dt2.msg,
    };
  ...
  if (res_dt.suc > 0) {
    if (res_dt.msg.length > 0) {
      res.send({
        ...
        status: "Data found",
        msg: res_dt.msg,
        board_member: bord_member_detail.msg,
      });
    } else {
      result = { suc: 0, status: "Data no found", msg: res_dt, data: req.body };
      res.send(result);
...
WapiRouter.post("/rangedtllist", async (req, res) => {
  var select = "*",
    table_name = "td_range_detail",
    where = null,
    order = null;
  var res_dt = await db_Select(select, table_name, where, order);
  if (res_dt.suc > 0) {
